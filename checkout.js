  ...
  if (req.method === "POST") {
    ...
    if (event.type === "checkout.session.completed") {
      ...
      const downloadLinks = purchasedItems
        .map((item) => `<li><strong>${item.name}</strong><br><a href="${item.url}">Download ${item.name}</a></li>`)
        .join("");
      ...
      await resend.emails.send({
        ...
        html: `
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
            ...
            <p style="color: #888; font-size: 14px;">If you have any questions, reply to this email or contact us at info@mbartype.com</p>
